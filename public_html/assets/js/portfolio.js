...
window.onload = (event) => {
	fetch('http://bellum4n.beget.tech/api/getLogin', {
		method: 'GET',
	})
	...
	fetch('http://bellum4n.beget.tech/api/portfolio', {
		method: 'GET',
	})
	.then((response) => response.json())
	.then((data) => {
...
function getActives(portfolioId){
	fetch('http://bellum4n.beget.tech/api/showCryptocurrency', {
		method: 'POST',
		...
		portfolioContent.innerHTML += `
			<div class="portfolio__crypto">
				<div class="portfolio__crypto-logo">
					<img src="../assets/images/crypto/${CryptocurrencyInfo[key].picture}" alt="Cryptocurrency logo">
				</div>
	...
	ws.onmessage = (event) => {
		if ((JSON.parse(event?.data)?.data?.p) && (JSON.parse(event.data).data.p != undefined))
		{
			...
			if (!isNaN(summ)) {
				...
				portfolioCost.textContent = 'Прогружаем...';
			}
		}
	}
}
...
function updateCryptocurrencyAmount(cryptocurrencyUniqueId) {
	updateButton.addEventListener('click', (event) => {
		event.preventDefault();
		fetch('http://bellum4n.beget.tech/api/updateActive', {
			method: 'POST',
			body: JSON.stringify({
...
function removeCryptocurrency(cryptocurrencyUniqueId) {
	removeButton.addEventListener('click', (event) => {
		event.preventDefault();
		fetch('http://bellum4n.beget.tech/api/removeActive', {
			method: 'POST',
			body: JSON.stringify({
				cryptocurrencyUniqueId: cryptocurrencyUniqueId,
			}),
...
logoutButton.addEventListener('click', (event) => {
	event.preventDefault();
	fetch('http://bellum4n.beget.tech/api/logout', {
		method: 'PUT',
	})
	.then((response) => {
		response.json();
		if (response.ok) {
			...
			window.location = 'http://bellum4n.beget.tech/';
		}
	})
});
...
searchInput.addEventListener('input', (event) => {
	...
	if (event.target.value != '') {
		...
		fetch('http://bellum4n.beget.tech/api/searchCryptocurrency', {
			method: 'POST',
		...
		searchResultBlock.innerHTML += (`
			<div class="search__result-crypto" id="result_crypto" data-unique-id="${cryptocurrency[key]['unique_id']}">
				<div class="search__result-logo">
					<img src="../assets/images/crypto/${cryptocurrency[key]['picture']}" alt="" class="result-logo">
				</div>
...
function addCryptocurrency() {
	addButton.addEventListener('click', (event) => {
		...
		fetch('http://bellum4n.beget.tech/api/addActive', {
			method: 'POST',
			body: JSON.stringify(data),
			headers: {
				'Content-type': 'application/json; charset="UTF-8"',
